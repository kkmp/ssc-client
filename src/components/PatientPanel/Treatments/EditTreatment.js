...
const EditTreatment = (treatment) => {
  ...
  useEffect(() => {
    const handleChange = () => {
      ...
      setTreatmentStatus({
        value: treatment.data.treatmentStatusId,
        label: treatment.data.treatmentStatus,
      });
      if (treatment.data.isCovid !== null) {
        var resultSearch = isCovidOptions.filter(
          (item) => item.value === treatment.data.isCovid
        );
