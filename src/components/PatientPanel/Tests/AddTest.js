...
const AddTest = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!testType) {
      setError({
        errors: { message: ["Aby zapisać, należy wybrać typ testu"] },
      });
      return;
    }
    ...
    if (!place) {
      setError({
        errors: { message: ["Aby zapisać, należy wybrać placówkę"] },
      });
      return;
    }
    ...
    if (result.value && resultDate) {
      ...
      setError({
        errors: {
          message: [
            "Aby zapisać, należy podać datę wyniku i wybrać rodzaj wyniku",
          ],
        },
      });
      return;
  ...
  return (
    <Fragment>
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-outline mb-4">
          ...
          <input
            ...
            maxLength={12}
            minLength={12}
            placeholder={"Podaj numer testu"}
          />
        </div>

        <div className="form-outline mb-4">
          <label className="form-label" htmlFor="testDate">
        ...
        <div className="form-outline mb-4">
          ...
          <div className="form-outline mb-4">
            <Select
              id="result"
              name="result"
              placeholder="Wybierz wynik testu"
              value={result}
              onChange={setResult}
        ...
        <div className="form-outline mb-4">
          <label className="form-label" htmlFor="resultDate">
          ...
          <input
            ...
            name="resultDate"
            value={resultDate}
            onChange={({ target }) => setResultDate(target.value)}
            className="form-control"
          />
        </div>
      ...
      <Popup
        component={<AddPlace onSubmit={handleChange} />}
        trigger={buttonPopup}
